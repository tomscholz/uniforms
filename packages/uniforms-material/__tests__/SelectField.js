...
test("<SelectField> - renders a SelectField with correct options", () => {
  ...
  expect(
    wrapper.find(SelectFieldMaterial).prop("children")[0].props.primaryText
  ).toBe("a");
  expect(
    wrapper.find(SelectFieldMaterial).prop("children")[1].props.primaryText
  ).toBe("b");
});
...
test("<SelectField checkboxes> - renders a set of Checkboxes which correct labels", () => {
  // eslint-disable-line max-len
  const onChange = jest.fn();
