...
test('<ErrorsField> - renders list of correct error messages (context)', () => {
    ...
    expect(wrapper.find('.panel-body > div')).toHaveLength(3);
    expect(wrapper.find('.panel-body > div').at(0).text()).toBe('X is required');
    expect(wrapper.find('.panel-body > div').at(1).text()).toBe('Y is required');
